...
void floodFill4(int x,int y,float fillCol[3],float intCol[3]){
    float color[3];
    getPixel(x,y,color);
    bool isFill = true;
    for(int i=0;i<3;i++){           // is eqal to intCol
        if(color[i]-intCol[i] != 0){
            //std::cout << color[i] << " i-"<< i<< " x-"<< x<< " y-"<< y << std::endl;
            isFill =false;
            break;
        }
    }
    ...
    if(isFill){
        ...
        floodFill4(x+1,y,fillCol,intCol);
        floodFill4(x-1,y,fillCol,intCol);
        floodFill4(x,y+1,fillCol,intCol);
        floodFill4(x,y-1,fillCol,intCol);
    }
}
...
void mouse(int btn, int state, int x, int y)
{
	if(btn==GLUT_LEFT_BUTTON && state == GLUT_DOWN) 
	{
		int xi = x;
		int yi = (wh-y);
        floodFill4(160,260,fillCol,intCol);
	}
}
